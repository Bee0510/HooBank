...
const NavBar = () => {
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center flex-1 xl:ml-64">
      <img src={logo} alt="hoobank " className="w-[124px] h-[32px]" />
      <ul className=" list-none justify-center items-center sm:flex hidden">
        {navLinks.map((nav, index) => (
          <li
            key={nav.id}
            className={`cursor-pointer font-poppins font-bold text-[16px] hover:text-[#00f6ff]  ${
      ...
      <div className=" sm:hidden flex flex-1 justify-end items-center">
        ...
        <div
          className={`${
            toggle ? "flex" : "hidden"
          } p-6 bg-gradient-to-t from-gray-400 to-slate-800 sticky top-20 right-1 mx-4 my-2 min-w-[140px] rounded-xl sidebar`}
        >
